...
def get_symbol_info(symbol='BTC-USDT-SWAP'):
    ...
    try:
        ...
        tick_size = float(info['data'][0].get('tickSz', 0.1))  # 获取价格精度
        return ct_val, min_qty, tick_size
    except Exception as e:
...
def place_order(side, pos_side, quantity, leverage, margin_mode, take_profit=None, stop_loss=None, tp_sl_mode='exchange'):
    try:
        ct_val, min_qty, tick_size = get_symbol_info('BTC-USDT-SWAP')
        quantity = round(quantity / ct_val, 2)
        if quantity < min_qty:
        ...
        cl_ord_id = f"order{int(time.time())}"
        params = {
        ...
        if tp_sl_mode == 'exchange' and take_profit is not None and stop_loss is not None:
            ...
            if pos_side == 'long':
                tp_trigger_price = round(current_price * (1 + take_profit / 100) / tick_size) * tick_size
                sl_trigger_price = round(current_price * (1 - stop_loss / 100) / tick_size) * tick_size
            else:
                tp_trigger_price = round(current_price * (1 - take_profit / 100) / tick_size) * tick_size
                sl_trigger_price = round(current_price * (1 + stop_loss / 100) / tick_size) * tick_size

            if pos_side == 'long' and sl_trigger_price >= current_price:
                raise ValueError(f"多头止损价格 {sl_trigger_price} 必须低于当前价格 {current_price}")
            if pos_side == 'short' and sl_trigger_price <= current_price:
                raise ValueError(f"空头止损价格 {sl_trigger_price} 必须高于当前价格 {current_price}")

            algo_cl_ord_id = f"tpsl{int(time.time())}"
            algo_order = {
                'tpTriggerPx': f"{tp_trigger_price:.{precision}f}",
                'tpOrdPx': '-1',
                'tpTriggerPxType': 'last',
                'tpOrdKind': 'condition',
                'slTriggerPx': f"{sl_trigger_price:.{precision}f}",
                'slOrdPx': '-1',
                'slTriggerPxType': 'last',
                'attachAlgoClOrdId': algo_cl_ord_id
            }
            params['attachAlgoOrds'] = [algo_order]
        ...
        if order['code'] == '0':
            ...
            tp_sl_text = f", 止盈止损: {tp_sl_mode}" if tp_sl_mode == 'exchange' else ""
            update_queue.put(("status_label", f"{action} 订单已下: 数量 {quantity} 张{tp_sl_text}"))
            logging.info(f"{action} 订单已下: {quantity} 张{tp_sl_text}, params: {params}")
            return order['data'][0]['ordId']
        else:
            update_queue.put(("status_label", f"下单失败: {order['msg']}"))
            logging.error(f"下单失败: {order['msg']}, params: {params}")
            return None
    except Exception as e:
